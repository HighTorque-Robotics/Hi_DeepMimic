...
# ####### custom cost:
@Cost.create_factory
def foot_slippage_cost(
    vals: VarValues,
    robot: Robot,
    joint_var: Var[Array],
    T_world_base_var: Var[jaxlie.SE3],
    prev_foot_pose: jaxlie.SE3,
    foot_link_indices: Array,
    pos_weight: Array | float,
) -> Array:
    """Computes the residual for matching link poses relative to a mobile base."""
    assert foot_link_indices.dtype == jnp.int32
    joint_cfg = vals[joint_var]
    T_world_base = vals[T_world_base_var]
    Ts_base_link = robot.forward_kinematics(joint_cfg)  # FK is T_base_link
    T_base_target_link = jaxlie.SE3(Ts_base_link[..., foot_link_indices, :])
    T_world_target_link_actual = T_world_base @ T_base_target_link

    residual = (T_world_target_link_actual.inverse() @ prev_foot_pose).log()
    pos_residual = residual[..., :3] * pos_weight
    return jnp.concatenate([pos_residual]).flatten()

@Cost.create_factory
def foot_clearance_cost(
    vals: VarValues,
    robot: Robot,
    joint_var: Var[Array],
    T_world_base_var: Var[jaxlie.SE3],
    foot_link_indices: Array,
    prev_foot_pose: jaxlie.SE3,
    height_weight: Array | float,
    slip_weight: Array | float
) -> Array:
    ...
    foot_height = T_world_target_link_actual.translation()[:, 2]
    foot_interact = jnp.maximum(0.0, -foot_height)

    residual = (T_world_target_link_actual.inverse() @ prev_foot_pose).log()
    pos_residual_xy = residual[..., :2] 
    clearance = foot_interact @ pos_residual_xy
    # import ipdb; ipdb.set_trace();
    return jnp.concatenate([foot_interact * height_weight,
                            clearance * slip_weight]).flatten()

@Cost.create_factory
def my_smoothness_cost(
    vals: VarValues,
    curr_joint_var: Var[Array],
    prev_joint: Array,
    weight: Array | float,
    ...
    """Computes the residual penalizing joint configuration differences (velocity)."""
    return ((vals[curr_joint_var] - prev_joint) * weight).flatten()
