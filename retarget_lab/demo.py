...
def render_incam(cfg):
    ...
    for i in tqdm(range(total_frames), desc=f"Rendering Incam"):
...
if __name__ == "__main__":
    ...
    if Path(paths.hmr4d_results).exists():
        result = torch.load(paths.hmr4d_results, weights_only=True)
        result = result['smpl_params_global']

        from scipy.spatial.transform import Rotation as sRot
        global_orient = result['global_orient']
        roll_angle = np.pi / 2
        roll_axis = np.array([1, 0, 0])
        roll_rotation = sRot.from_rotvec(roll_angle * roll_axis)
        ori_rotations = sRot.from_rotvec(global_orient)
        new_ori_rotations = roll_rotation * ori_rotations
        new_global_orient = new_ori_rotations.as_rotvec()
        new_trans = roll_rotation.apply(result['transl'].cpu().numpy())

        pose = np.concatenate([new_global_orient, result['body_pose']], axis=1) 
        trans = new_trans
        betas = result['betas'].cpu().numpy()

        phc_data = {}
        phc_data['poses'] = pose
        phc_data['betas'] = betas
        phc_data['trans'] = trans
        phc_data['gender'] = 'male'
        phc_data['mocap_framerate'] = 50

        # print(pose.shape)
        # exit()

        pkl_path = './outputs/demo/phc_data.pkl'
        with open(pkl_path, 'wb') as f:
            pickle.dump(phc_data, f, protocol=pickle.HIGHEST_PROTOCOL)

        npz_path = './outputs/demo/phc_data.npz'
