...
class hi_config:
    ...
    assets_path = "assets/urdf/hi/urdf/hi_new.urdf"
    mesh_dir = "assets/urdf/hi/meshes"
    target_link_names = ['waist_link',  'l_hip_pitch_link', "l_calf_link", "l_ankle_roll_link",  \
...
def main():
    ...
    # 加载预处理数据
    hi_config.target_link_names = loaded_data["link_name"]
    # print(hi_config.target_link_names)
    fps = loaded_data["fps"]
    ...
    target_positions[:, :, 2] -= (min_foot_height + 0.06) #将整体高度降低

    target_wxyzs = onp.zeros((target_positions.shape[0], target_positions.shape[1], 4))
    target_wxyzs[:, :, 0] = 1.0
    target_wxyzs[:, [hi_config.target_link_names.index(hi_config.toe_names[0]), \
                     hi_config.target_link_names.index(hi_config.toe_names[1])]] = toe_wxyzs

    hi_config.pos_weight = onp.ones((len(hi_config.target_link_names), 3)) * 5.0
    hi_config.ori_weight = onp.zeros((len(hi_config.target_link_names), 3))
    hi_config.ori_weight[hi_config.target_link_names.index(hi_config.toe_names[0]), :] = 1.0
    hi_config.ori_weight[hi_config.target_link_names.index(hi_config.toe_names[1]), :] = 1.0

##############################################################################################
    # Create robot.
    urdf = yourdfpy.URDF.load(hi_config.assets_path, mesh_dir=hi_config.mesh_dir)
    ...
    prev_foot_pos = onp.zeros((len(hi_config.foot_names), 3))
    prev_foot_wxyz = onp.zeros((len(hi_config.foot_names), 4))
    ...
    while idx != target_positions.shape[0]:
        ...
        base_pos, base_wxyz, cfg = pks.solve_retargeting (
            ...
            target_link_names = hi_config.target_link_names,
            foot_link_names = hi_config.foot_names,
            ...
            link_pos_weight=hi_config.pos_weight,
            link_ori_weight=hi_config.ori_weight,
        )
