...
def solve_retargeting(
    ...
    prev_foot_pos: onp.ndarray,
    foot_link_indices: onp.ndarray,
    
    pos_weight: onp.ndarray = onp.array(5.0),
    ori_weight: onp.ndarray = onp.array(0.0),
    ...
    base_pose, cfg = _solve_ik_jax(
        ...
        jnp.array(prev_wxyz),
        jnp.array(prev_cfg),
        jnp.array(prev_foot_pos),
        jnp.array(foot_link_indices),
        jnp.array(pos_weight),
        jnp.array(ori_weight), 
        first_frame,
    )
...
def _solve_ik_jax(
    ...
    factors = [
        ...
        pk.costs.rest_with_base_cost(
            joint_var,
            base_var,
            jnp.array(joint_var.default_factory()) * first_frame,
            jnp.array(
